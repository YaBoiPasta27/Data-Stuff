...
@st.cache_data
def generate_eupp_data():
    """Generate Energy Use Per Person (EUPP) data for multiple countries"""
    countries = ['India', 'China', 'United States', 'Russia', 'Japan', 
                'Germany', 'Iran', 'South Korea', 'Saudi Arabia', 'Canada']
    data = []
    
    np.random.seed(43)  # Different seed for EUPP data
    
    # Base EUPP values (in GJ per capita per year)
    base_eupp = {
        'United States': 300,
        'Canada': 280,
        'Saudi Arabia': 250,
    ...
    for year in range(1980, 2015):
        for country in countries:
            base_value = base_eupp[country]
            
            # Different growth patterns for different countries
            if country in ['China', 'India']:
                growth_factor = 1 + (year - 1980) * 0.045  # Faster growth
            elif country in ['United States', 'Canada']:
                growth_factor = 1 + (year - 1980) * 0.008  # Slower growth
            else:
                growth_factor = 1 + (year - 1980) * 0.015  # Moderate growth
            
            # Add some cyclical variation and noise
            cyclical = 1 + 0.05 * np.sin((year - 1980) * 0.5)
            noise = np.random.normal(1, 0.08)
            
            value = base_value * growth_factor * cyclical * noise
            
            data.append({
                'country': country,
                'year': year,
                'eupp': max(10, value),  # Minimum 10 GJ per capita
                'log_eupp': np.log10(max(10, value))
            })
...
def generate_gdp_data():
    """Generate GDP change data for multiple countries"""
    countries = ['India', 'China', 'United States', 'Russia', 'Japan', 
                'Germany', 'Iran', 'South Korea', 'Saudi Arabia', 'Canada']
    data = []
    
    np.random.seed(44)  # Different seed for GDP data
    
    # Base GDP growth rates (annual %)
    base_gdp_growth = {
        'China': 9.5,
        'India': 6.5,
        'South Korea': 5.5,
    ...
    for year in range(1980, 2015):
        for country in countries:
            base_growth = base_gdp_growth[country]
            
            # Add economic cycles and external shocks
            if year in [1991, 2008, 2009]:  # Recession years
                shock = -3.0 if country in ['United States', 'Germany', 'Japan'] else -1.5
            elif year in [1992, 1993, 2010]:  # Recovery years
                shock = 2.0 if country in ['United States', 'Germany', 'Japan'] else 1.0
            else:
                shock = 0
            
            # Economic cycle (longer term)
            cycle = 1.5 * np.sin((year - 1980) * 0.3)
            
            # Random variation
            noise = np.random.normal(0, 1.2)
            
            gdp_change = base_growth + cycle + shock + noise
            
            # Ensure reasonable bounds
            gdp_change = max(-8.0, min(15.0, gdp_change))
            
            data.append({
                'country': country,
                'year': year,
                'gdp_change': round(gdp_change, 2)
            })
...
df_eupp = generate_eupp_data()
df_gdp = generate_gdp_data()
...
for country in countries:
    ...
    y_values = country_data['log_eupp'] if show_log_scale else country_data['eupp']
...
fig_eupp.update_layout(
    title=f"Energy Use Per Person Trends ({year_range[0]}-{year_range[1]})",
    xaxis_title="Year",
...
# NEW CHART 2: GDP Change Over Time
...
fig_gdp.update_layout(
    title=f"GDP Growth Rate Trends ({year_range[0]}-{year_range[1]})",
    xaxis_title="Year",
...
st.sidebar.info(
    ...
    "CO2 emissions trends, temperature correlations, EUPP analysis, "
    "GDP growth patterns, and regional analysis features."
)
